...
bool String::operator==(char *str)const
{
	...
	int i;
	for (i=0;strr[i]==str[i];i++);
	if (i==len)return true;
	return false;
}
int String::operator ()(char aa)
{
	return find(aa);
}
String &String::operator*(int k)
{
	char *rstr=new char[len*k];
	for (int i=0;i<len;i++)
		rstr[i]=strr[i];
	for (int m=1;m<k;m++)
	{
		for (int i=0,j=len*m;i<len;i++,j++)
		{
			rstr[j]=strr[i];
		}
	}
...
char *String::getstr(){ return strr;}
String::~String() {
